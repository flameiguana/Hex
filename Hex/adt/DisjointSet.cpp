...
class DisjointSet::SimpleNode{
	...
	DisjointSet::~DisjointSet(){
		std::vector<std::list<SimpleNode*>*>::iterator setsIt;
		for(setsIt = sets.begin(); setsIt != sets.end(); ++setsIt)
		{
			std::list<SimpleNode*>::iterator nodeIt;
			for(nodeIt = (**setsIt).begin(); nodeIt != (**setsIt).end(); ++nodeIt)
			{
				//delete all nodes
				delete *nodeIt;
			}
			//delete the list itself once all nodes are deleted
			delete *setsIt;
		}
	}
}